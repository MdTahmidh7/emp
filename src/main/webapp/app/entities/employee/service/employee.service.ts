...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'empBirthDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      empBirthDate: employee.empBirthDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      empBirthDate: restEmployee.empBirthDate ? dayjs(restEmployee.empBirthDate) : undefined,
    };
  }
