...
describe('Employee Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Address query and add missing value', () => {
      ...
      const employeeAddress: IAddress = { id: 35307 };
      employee.employeeAddress = employeeAddress;

      const addressCollection: IAddress[] = [{ id: 39134 }];
      jest.spyOn(addressService, 'query').mockReturnValue(of(new HttpResponse({ body: addressCollection })));
      const additionalAddresses = [employeeAddress];
    ...
    it('Should update editForm', () => {
      const employee: IEmployee = { id: 456 };
      const employeeAddress: IAddress = { id: 7835 };
      employee.employeeAddress = employeeAddress;

      activatedRoute.data = of({ employee });
      comp.ngOnInit();

      expect(comp.addressesSharedCollection).toContain(employeeAddress);
